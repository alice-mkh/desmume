...
static u8 Mic_GenerateCustomMicSample(void)
{
	static unsigned int i = 0;

	if (++i >= G_N_ELEMENTS(customMicSample)) {
		i = 0;
	}

	return customMicSample[i];
}
...
void Mic_DoNoise(BOOL noise)
{
	...
	} else if (CommonSettings.micMode == MicMode_Sample) {
		generator = &Mic_GenerateCustomMicSample;
	}
